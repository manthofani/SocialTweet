...
const Content = () => {
    ...
    useEffect(() => {
        ...
        if (!username){
            tweetService.getPublicTweets().then(res => setTweets(res.data))
        }else{
    ...
    return (
        <main className="flex flex-col border-r border-l border-gray-extraLight w-1/2 mr-auto">
            <header
                className="sticky top-0 z-10 flex justify-between items-center p-4 h-16 border-b border-gray-extraLight">
                <span className="font-bold text-xl text-gray-900">{username ? 'Home' : 'Recent Tweet'}</span>
