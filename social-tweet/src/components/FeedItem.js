    ...
    return (
        <>
            <article className="flex space-x-3 border-b border-gray-extraLight px-4 py-3 cursor-pointer">
                <img src={ userProfileImageLink || isComment ? `http://localhost:8080/v1/users/${username}/image/download` : profile}
                ...
                <div className="flex-1">
                    <div className="flex text-sm justify-between">
                        ...
                        {new Date(creationTimestamp).toLocaleDateString("id-ID",  { weekday: 'long' }) } ,
                        {new Date(creationTimestamp).toLocaleString("en-EN")}
                        </span>
                        </div>
                        { isComment ? '' : (currentUser === username ) ? 
                        <div className=" rounded-full items-center">
                        <MoreHorizIcon className=" w-5 h-5" onClick={handleClick}/>
                        {isVisible && <DropdownMenu tweetId={id} text={text} />}                                
                        </div> : ''
                        }
                    </div>
